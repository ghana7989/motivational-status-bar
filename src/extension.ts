...
export function activate(context: vscode.ExtensionContext) {
  ...
  let add = vscode.commands.registerCommand("fun.start", () => {
    ...
    // statusBarItem.text will be added to the VS Code status bar
    statusBarItem.text = `<add-something-cool-here>`;

    statusBarItem.show();
  });

  let remove = vscode.commands.registerCommand("fun.end", () => {
    if (chains.length > 0) {
